...
def test_cli_invalid_path() -> None:
    result = runner.invoke(app, ["/nonexistent/__dirplot_test__", "--no-show"])
...
def test_cli_not_a_directory(tmp_path: Path) -> None:
    f = tmp_path / "file.txt"
    f.write_text("hello")
    result = runner.invoke(app, [str(f), "--no-show"])
    assert result.exit_code == 1
    assert "Not a directory" in result.output


def test_cli_bad_colormap(sample_tree: Path) -> None:
    result = runner.invoke(app, [str(sample_tree), "--no-show", "--colormap", "not_a_cmap"])
    assert result.exit_code == 1
    assert "Unknown colormap" in result.output


def test_cli_runs_successfully(sample_tree: Path) -> None:
    result = runner.invoke(app, [str(sample_tree), "--no-show"])
...
def test_cli_saves_output(sample_tree: Path, tmp_path: Path) -> None:
    ...
    result = runner.invoke(app, [str(sample_tree), "--no-show", "--output", str(output)])
    assert result.exit_code == 0
...
def test_cli_exclude(sample_tree: Path) -> None:
    src = sample_tree / "src"
    result = runner.invoke(app, [str(sample_tree), "--no-show", "--exclude", str(src)])
    assert result.exit_code == 0
...
def test_cli_show_window(sample_tree: Path) -> None:
    ...
    with patch("PIL.Image.open", return_value=mock_img):
        result = runner.invoke(app, [str(sample_tree), "--show"])
    assert result.exit_code == 0
...
def test_cli_show_inline(sample_tree: Path) -> None:
    ...
    with patch("builtins.open", m):
        result = runner.invoke(app, [str(sample_tree), "--show", "--inline"])
    assert result.exit_code == 0


def test_cli_custom_colormap(sample_tree: Path) -> None:
    result = runner.invoke(app, [str(sample_tree), "--no-show", "--colormap", "viridis"])
    assert result.exit_code == 0


def test_cli_custom_scale(sample_tree: Path) -> None:
    result = runner.invoke(app, [str(sample_tree), "--no-show", "--font-size", "14"])
    assert result.exit_code == 0


def test_cli_custom_size(sample_tree: Path) -> None:
    result = runner.invoke(app, [str(sample_tree), "--no-show", "--size", "800x600"])
    assert result.exit_code == 0
    assert "800x600" in result.output


def test_cli_invalid_size(sample_tree: Path) -> None:
    result = runner.invoke(app, [str(sample_tree), "--no-show", "--size", "notasize"])
    assert result.exit_code == 1
