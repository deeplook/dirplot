    ...
    def iterdir(self) -> Iterator[VirtualPath]:
        ...
        for info in self._archive.infolist():
            if info.filename.startswith(prefix):
                ...
                if "/" in rest:
                    # This is a nested directory, get first component
...
class TarMember:
    ...
    def is_dir(self) -> bool:
        """Check if this member is a directory."""
        member = self._archive.getmember(self._name) if self._name in self._archive.getnames() else None
        if member and member.isdir():
            return True
        # Or check if any entry has this as prefix
        for m in self._archive.getmembers():
            if m.name.startswith(self._name + "/"):
                return True
        return False
...
class ArchiveRoot:
    ...
    def __enter__(self) -> ArchiveRoot:
        ...
        if name.endswith(".zip") or any(name.endswith(ext) for ext in 
            [".jar", ".war", ".ear", ".whl", ".apk", ".epub"]):
